...
import com.netease.lowcode.extension.UploadPatchErrorCodeEnum;
import com.netease.lowcode.extension.UploadPatchFileUploadUtils;
import com.netease.lowcode.extension.UploadPatchHttpCodeException;
...
@Component
public class UploadPatchS3FileStorageClient implements UploadPatchFileStorageClient {
    private static final Logger LOGGER = LoggerFactory.getLogger(UploadPatchS3FileStorageClient.class);
    ...
    public void init(Map<String, String> config) {
        storageType = ObjectUtils.defaultIfNull(config.get(UploadPatchConstants.UPLOAD_CONFIG_KEY_FILE_STORAGE_TYPE), S3_TYPE);
        address = config.get(UploadPatchConstants.UPLOAD_CONFIG_KEY_S3_ADDRESS);
        accessKey = config.get(UploadPatchConstants.UPLOAD_CONFIG_KEY_S3_ACCESS_KEY);
        secretKey = config.get(UploadPatchConstants.UPLOAD_CONFIG_KEY_S3_SECRET_KEY);
        bucket = config.get(UploadPatchConstants.UPLOAD_CONFIG_KEY_S3_BUCKET);
        cdnAddress = config.get(UploadPatchConstants.UPLOAD_CONFIG_KEY_CDN_ADDRESS);
    ...
    public String upload(InputStream inputStream, String targetPath, Map<String, String> payloads) throws IOException {
        if (null == s3Client) {
            throw new UploadPatchHttpCodeException(HttpStatus.METHOD_NOT_ALLOWED.value(), UploadPatchErrorCodeEnum.FILESYSTEM_NOT_SUPPORT.code);
        }
        ...
        metadata.setContentType(UploadPatchFileUploadUtils.getFileMimeType(key));
    ...
    public void download(OutputStream outputStream, String targetPath, Map<String, String> payloads) throws IOException {
        ...
        try {
            ...
            //批量下载文件时, 不应该设置
            if (payloads.containsKey(UploadPatchFileStorageClient.SINGLE_DOWNLOAD) && Boolean.valueOf(payloads.get(UploadPatchFileStorageClient.SINGLE_DOWNLOAD))) {
            ...
            if("NoSuchKey".equals(e.getErrorCode())){
                throw new UploadPatchHttpCodeException(String.format("文件不存在 NoSuchKey{%s}",targetPath));
            }else {
                throw new UploadPatchHttpCodeException(e.getErrorMessage());
            }
        }
    }
    ...
    private URL getTargetUrl(String targetPath, String bucket, AmazonS3 targetClient) {
        if (UploadPatchConstants.UPLOAD_ACCESS_PRIVATE.equals(this.access)) {
            //需要加签

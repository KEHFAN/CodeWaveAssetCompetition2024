...
import static com.netease.lowcode.extension.filestorage.UploadPatchFileStorageClient.DOWNLOAD_FORM_ORIGINAL_S3_CLIENT;

public class UploadPatchFileUploadUtils {
    ...
    private static final int EXPIRATION_START_INDEX = (UploadPatchConstants.UPLOAD_ACCESS_SUFFIX_MAGIC + UploadPatchConstants.UPLOAD_ACCESS_PRIVATE_SUFFIX).length();
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(UploadPatchFileUploadUtils.class);
    ...
    public static Triple<String/*原始文件名称*/, String/*处理过后的文件名*/, String/*处理过后的文件名 + 后缀*/> handlerFileName(String filePath, Boolean hashFileName, String appId, Boolean needEncrypt) throws Exception {
        ...
        //对象存储中的文件名需要和返回的不一致
        filePath = UploadPatchFileUploadUtils.replaceEncryptEndTag(filePath);
...
//        }
        ...
        //对文件名加密
        String encodeFileName = UploadPatchFileUploadUtils.encryptFilePath(URLEncoder.encode(originalFileName, "UTF-8"));
    ...
    public static String generateSavePath(String originPath, String basePath, String access, Double ttl,Boolean viaOriginUrl) {
        ...
        // 2. 添加权限后缀
        String permissionSuffix = UploadPatchFileUploadUtils.getPathPermission(access, ttl);

        // 3. 添加CDN加速后缀
        String cdnSuffix = BooleanUtils.isTrue(viaOriginUrl)? "_ori" : "";

        String saveFilename = filenameWithoutExt + UploadPatchConstants.UPLOAD_ACCESS_SUFFIX_JOINER + curTime + cdnSuffix+ permissionSuffix + ext;
    ...
    public static String getPathPermission(String access, Double ttl) {
        // 公开且不过期的文件不添加额外的后缀
        if (UploadPatchConstants.UPLOAD_ACCESS_PUBLIC.equalsIgnoreCase(access) && UploadPatchConstants.UPLOAD_ACCESS_NONE_EXPIRATION.equals(ttl)) {
            return "";
        }

        // 指定了文件访问权限信息，将在保存的文件名上添加权限后缀信息,生成规则: pms1/2[0时区到期时间戳]
        String accessPart = UploadPatchConstants.UPLOAD_ACCESS_PUBLIC.equalsIgnoreCase(access) ?
                UploadPatchConstants.UPLOAD_ACCESS_PUBLIC_SUFFIX : UploadPatchConstants.UPLOAD_ACCESS_PRIVATE_SUFFIX;
        String ttlPart = "";
        if (!UploadPatchConstants.UPLOAD_ACCESS_NONE_EXPIRATION.equals(ttl) && ttl > 0) {
            ttlPart = ZonedDateTime.now().withZoneSameInstant(ZoneId.of("UTC"))
                    .plusMinutes((long) (ttl * 24 * 60)).toInstant().toEpochMilli() + "";
        }

        return UploadPatchConstants.UPLOAD_ACCESS_SUFFIX_JOINER + UploadPatchConstants.UPLOAD_ACCESS_SUFFIX_MAGIC + accessPart + ttlPart;
    }
    ...
    public static boolean shouldAccessControl(String apiPath) {
        return StringUtils.isNotBlank(apiPath) && getPathPermissionSuffix(apiPath)
                .startsWith(UploadPatchConstants.UPLOAD_ACCESS_SUFFIX_MAGIC + UploadPatchConstants.UPLOAD_ACCESS_PRIVATE_SUFFIX);
    }
    ...
    public static boolean isPathExpiration(String apiPath) {
        ...
        if (StringUtils.isNotBlank(permissionSuffix) &&
                permissionSuffix.startsWith(UploadPatchConstants.UPLOAD_ACCESS_SUFFIX_MAGIC) &&
    ...
    public static String getPathPermissionSuffix(String apiPath) {
        ...
        String[] parts = nameWithoutExt.split(UploadPatchConstants.UPLOAD_ACCESS_SUFFIX_JOINER);
    ...
    public static String replaceEncryptEndTag(String fileName) {
        if (!fileName.contains(UploadPatchFileUploadUtils.ENCRYPT_FILE_PTAH)) {
            return fileName;
        }
        fileName = fileName.replace(UploadPatchFileUploadUtils.ENCRYPT_FILE_PTAH, "");
        if (fileName.equals("")) {
            //这里应该不允许仅有_efp的文件名
    ...
    public static String replaceTargetFileSeg(final String filePath, final String originSeg, final String targetSeg, Boolean needEncrypt) {
        ...
        return filePath.replace(originSeg, targetSeg + UploadPatchFileUploadUtils.ENCRYPT_FILE_PTAH + getRandomTag());
    }

    /**
     * 加密标识匹配
     */
    private final static Pattern pattern = Pattern.compile(UploadPatchFileUploadUtils.ENCRYPT_FILE_PTAH + "\\d{1}\\w{1}\\w{1}\\w{1}\\d{1}");
    ...
    public static String handlerEncryptFilePath(String filePath, Map<String, String> urlParams) throws Exception {
        ...
        int lastIndex = filePath.lastIndexOf(UploadPatchFileUploadUtils.ENCRYPT_FILE_PTAH);
        //_efpxxxx_{随机时间}
